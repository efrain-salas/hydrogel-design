...
__webpack_require__(/*! ./designer */ "./resources/js/designer.js");
...
document.addEventListener('livewire:load', function () {
  Livewire.on('pltFileReady', function (plt) {
    drawPlt(plt);
  });
...
window.safeAreaHeight = null; // STAGE
...
window.userBackgroundLayer = new Konva.Layer({//opacity: 0.8,
});
stage.add(userBackgroundLayer);
window.userForegroundLayer = new Konva.Layer();
stage.add(userForegroundLayer); // TRANSFORMER: Resize, etc

window.transformer = new Konva.Transformer();
userBackgroundLayer.add(transformer);
window.transformer = new Konva.Transformer();
userForegroundLayer.add(transformer); //userLayer.draw();
// FUNCTIONS

window.addImage = function (layer, url, x, y, width, height) {
  ...
  Konva.Image.fromURL(url, function (imageShape) {
    imageShape.setAttrs({
      x: x,
      y: y,
      width: width,
    ...
    imageShape.on('click tap', function (e) {
      imageShape.moveToTop();
      transformer.nodes([imageShape]);
      transformer.enabledAnchors(['top-left', //'top-center',
...
window.addEmoji = function (url) {
  addImage(userForegroundLayer, url, null, null, 100, 100);
};

window.addUserImage = function (layer) {
  ...
  reader.addEventListener('load', function () {
    ...
    img.onload = function () {
      ...
      var height = safeAreaWidth / aspectRatio;
...
window.addUserForegroundImage = function () {
  addUserImage(userForegroundLayer);
};

window.addUserBackgroundImage = function () {
  addUserImage(userBackgroundLayer);
};

window.addSvgPath = function (layer, pathString) {
...
window.addText = function () {
  ...
  textNode.on('click tap', function (e) {
    ...
    transformer.enabledAnchors([//'top-left',
    //'top-center',
    //'top-right',
  ...
  textNode.on('dblclick dbltap', function () {
    ...
    textNode.hide();
    transformer.hide(); // create textarea over canvas with absolute position
    // first we need to find position for textarea
    // how to find it?
    // at first lets find position of text node relative to the stage:

    var textPosition = textNode.absolutePosition(); // so position of textarea will be the sum of positions above:
    ...
    var px = 0; // also we need to slightly move textarea on firefox
    ...
    textarea.style.transform = transform; // reset height
...
window.changeSelectedTextFontFamily = function (fontFamily) {
...
window.changeSelectedTextFontSize = function (fontSize) {
...
window.changeSelectedTextAlignment = function (alignment) {
...
window.changeSelectedTextColor = function (color) {
...
window.getStageAsDataUrl = function () {
  frame.hide();
  plt.hide(); //userBackgroundLayer.opacity(1);

  var url = stage.toDataURL({
    pixelRatio: 4
  });
  frame.show();
  plt.show(); //userBackgroundLayer.opacity(0.8);

  return url;
};

window.downloadDataUrl = function (url) {
...
window.downloadStage = function () {
  downloadDataUrl(getStageAsDataUrl());
};

window.printStage = function () {
...
window.onNodeAdded = function () {
  setTimeout(function () {
    stage.draw();
  }, 200);
};

window.onContainerClicked = function () {
...
window.procLine = function (line) {
  ...
  if (i === line.length) return NaN;
  line = line.substr(i, line.length - i);
  var strs = line.split(',');
  var axis_x = parseInt(strs[0]);
  var axis_y = parseInt(strs[1]);
  if (isNaN(axis_x) || isNaN(axis_y)) return NaN;
  return {
    x: axis_x,
    y: axis_y,
    z: signZ
...
window.drawPlt = function (pltstr) {
  var width_edge = 20;
  var width_total = 119;
  var height_total = 185;
    ...
    sceneFunc: function sceneFunc(context, shape) {
      ...
      for (var i = 0; i < pltstr.length; i++) {
        ...
        if (ch === ';') {
          ...
          if (comma === 2) {
            var _point = procLine(line);

            if (_point) points.push(_point);
            comma = 0;
            line = "";
          }
        }
      ...
      for (var _i = 0; _i < points.length; _i++) {
        if (points[_i].x > max_x) max_x = points[_i].x;
        if (points[_i].y > max_y) max_y = points[_i].y;
        if (points[_i].x < min_x) min_x = points[_i].x;
        if (points[_i].y < min_y) min_y = points[_i].y;
      }

      var width = (max_x - min_x) / 40;
      var height = (max_y - min_y) / 40;
      var width_phone = width * const_ratio;
      var height_phone = height * const_ratio;
      context.beginPath();
      context.setLineDash([]);
      var center_x = (width_edge + width_box / 2) * const_ratio;
      var center_y = height_box / 2 * const_ratio;
      var offset_x = (width_box - width) / 2 * const_ratio;
      var offset_y = (height_box - height) / 2 * const_ratio;

      for (var _i2 = 0; _i2 < points.length; _i2++) {
        var x = (points[_i2].x - min_x) / 40 * const_ratio + offset_x;
        var y = (points[_i2].y - min_y) / 40 * const_ratio + offset_y;

        if (points[_i2].z) {
          var canvasX = width_total * const_ratio - Math.floor(x);
          ...
          var _canvasX = width_total * const_ratio - Math.floor(x);

          var _canvasY = Math.floor(y);

          context.moveTo(_canvasX, _canvasY);
        }
      }
